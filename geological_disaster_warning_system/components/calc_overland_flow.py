...
class CalcOverlandFlowView(QWidget):
    ...
    def _calc_t1(self):
        m1_str = MyUtils.safe_strip(self.t1.get('地表粗度系数m1', ''))
        Ls_str = MyUtils.safe_strip(self.t1.get('坡面流的长度Ls', ''))
        is_str = MyUtils.safe_strip(self.t1.get('坡面流的坡度is', ''))
        # 判断是否都存在且非空
    ...
    def _dynamic_form_value_change(self, dynamic_form_name,dynamic_component_name, val):
        if dynamic_form_name =="坡面汇流历时T1":
            ...
            logging.info(f"<UNK>dynamic_form_name:{dynamic_form_name};dynamic_component_name:{dynamic_component_name}--val:{val}-{self.t1}")
            self._calc_t1()
        ...
        for item in self.data:
            ...
            if tmp_type == "有子表":
                ...
                for key, value in table_value.items():
                    # value 是子表的ID
                    data1,table_name = self.model.get_sub_menu_content(value)
                    if table_name == "坡面汇流历时T1":
                        self.t1 = {item["name"]: '' for item in data1}
                        logging.info(f"<UNK> self.t1 :{ self.t1 }")
                    elif table_name == "沟内汇流历时T2":
                        self.t2 = {item["name"]: '' for item in data1}
                        logging.info(f"<UNK> self.t2 :{self.t2}")
                    logging.info(f"<UNK>{table_name} data: {data1}")
                    widget = DynamicFormWidget(data1, table_name)
    ...
    def set_data(self,data):
        for key, value in data.items():
            if key in self.value_widget:
                widget = self.value_widget[key]
                class_name = widget.__class__.__name__
                # 处理 DynamicFormWidget 类型控件
                if class_name == "DynamicFormWidget":
                    if isinstance(value, dict):
                        widget.set_values(value)
                        if key == "坡面汇流历时T1":
                            self.t1 = {k: v for k,v in value.items()}
                        elif key == "沟内汇流历时T2":
                            self.t2 = {k: v for k,v in value.items()}
                    continue

                # 处理 QLineEdit 类型控件
                if class_name == "QLineEdit":
                    tmp_text = str(value) if value is not None else ""
                    widget.setText(tmp_text)
                    self.overland_flow_data[key] = tmp_text
                    continue
                # 处理 QLabel 类型控件
            ...
            # # 特殊处理：非控件字段也尝试更新内部数据
            # if key in self.overland_flow_data:
            #     self.overland_flow_data[key] = value
