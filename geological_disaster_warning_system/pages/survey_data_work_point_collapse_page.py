...
from Model.survey_data_work_point_collapse_page_model import SurveyDataWorkPointCollapsePageModel
...
class SurveyDataWorkPointCollapsePage(MyBasePage):
    #TODO 以后对于特殊页面进行需要更改这里就可以了
    ...
    def create_model(self, main_window):
        return SurveyDataWorkPointCollapsePageModel(main_window.get_db_manager())
...
#--------------------------------------------------------------------------------------------------------------------------------------
    ...
    def _update_evaluation_content(self):
        ...
        for widget in self.value_widgets:
            # 获取菜单ID
            menu_id = widget.property("menu_id")
            if not menu_id:
                continue

            # 获取指标名称（只获取一次）
            menu_name = self.menu_name_map.get(menu_id, "")
            if not menu_name:
                continue
            ...
            if value:
                # 初始化该指标的存储（如果尚未存在）
                if menu_id not in evaluation_groups:
                    evaluation_groups[menu_id] = {
                        "name": menu_name,
                        "values": []
                    }
                evaluation_groups[menu_id]["values"].append(value)

        # 生成评价文本
        evaluation_items = []
        for group in evaluation_groups.values():
            # 将同一指标的所有值用逗号连接
            values_str = ", ".join(group["values"])
            evaluation_items.append(f"{group['name']}: {values_str}")

        # 更新指标评价标签
        evaluation_text = "; ".join(evaluation_items)
        evaluation_widget = self.content_controls.get(72)
    ...
    def _handle_table_selected(self,table_name):
        ...
        logging.info(f"<UNK>增加-----{mark_index}<UNK>")
    ...
    def on_ai_evaluation_finished(self, result):
        ...
        control_widget = self.content_controls[74]
    ...
    def bottom_button_clicked(self,item):
        ...
        if name == "智能分析":
            ...
            self.model.upsert_work_point_score(self.work_point_id,self.current_menu_id,self.menu_score[self.current_menu_id])
            self.update_menu_score()
        pass
